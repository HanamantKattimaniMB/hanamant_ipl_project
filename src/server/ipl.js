...
let createTableMatches = `CREATE TABLE IF NOT EXISTS matches(Id INT PRIMARY KEY,
                            season INT,
                            city VARCHAR(20),
                            date DATE,
...
let createTableDeliveries = `CREATE TABLE IF NOT EXISTS deliveries(match_id INT,
                              inning INT,
                              batting_team VARCHAR(60),
                              bowling_team VARCHAR(60),
                              over INT,ball INT,batsman VARCHAR(60),
...
let loadMatchesData=`LOAD DATA LOCAL INFILE '/home/lakan/MB/testing/hanamant_ipl_project/src/data/matches.csv' 
                      INTO TABLE matches 
...
let loadDeliveriesData=`LOAD DATA LOCAL INFILE '/home/lakan/MB/testing/hanamant_ipl_project/src/data/deliveries.csv'
...
function  matchesPerYear_sql(){
  return new Promise((resolve,reject) => {
    connection.query(`SELECT season,count(season) as numberOfMatches 
      FROM matches 
      GROUP BY season`, 

      (error, matchesPerYear) => {
          if(error){
            return reject(error)
          } else{
...
function  matchesOwnPerTeamPerYear_sql(){
  return new Promise((resolve,reject) => {
    connection.query(`SELECT season,winner,COUNT(winner) as numberOfMatches  
     FROM matches WHERE winner!="" 
     GROUP BY season,winner ORDER BY season`, 

      (error, matchesOwnPerTeamPerYear) => {
          if(error){
            return reject(error)
          } else{
            return resolve(matchesOwnPerTeamPerYear)
...
function  extraRunConceded_sql(){
  return new Promise((resolve,reject) => {
    connection.query(`SELECT d.bowling_team, SUM(d.extra_runs) as extra_runs
      FROM deliveries d JOIN matches m ON m.id = d.match_id WHERE m.season = 2016
      GROUP BY d.bowling_team`,

      (error, extraRunConceded) => {
        if(error){
          return reject(error)
        } else {
...
function  topTenEconomicalBowler_sql(){
  return new Promise((resolve,reject) => {
      connection.query(`SELECT bowler, ((runs*1.0)/(overs*1.0))AS over_runs 
        FROM(SELECT bowler , ((count(ball) *1.0) /(6 * 1.0))AS overs, SUM(total_runs)AS runs 
        FROM deliveries JOIN matches ON deliveries.match_id = matches.id  
        WHERE matches.season = 2015 AND wide_runs=0 AND noball_runs = 0 
        GROUP BY bowler) inter 
        ORDER BY over_runs LIMIT 10`, 

        (error, topTenEconomicalBowlers) => {
          if(error){
            return reject(error)
          } else {
